...
## 5b. Infrastructure — dedicated Cloudflare account & `oktryme.com` serving
...
### `oktryme.com` serving plan ("Phase B") — zone-move **abandoned**; reverse-proxy instead
`oktryme.com` lives in the **old** personal account and **stays there.** The original plan was a one-shot inter-account *zone move*; that was **abandoned (2026-06-26)** on discovery that `oktryme.com` was a **shared, multi-service zone** (18 records: a Next.js apex site, `prod/staging.pickle` apps, two Cloudflare Tunnels `bb`/`pallas`, `blog→carmen.blog`, GoDaddy Domain-Connect/NS leftovers, plus Email Routing + Resend). A whole-zone move drops all DNS and **can't re-create the Tunnels** (their resources live in the old account) — far too destructive for the benefit.

**Cleanup done (2026-06-26)** so `oktryme.com` is now a clean brand domain (via a temp old-account API token, since wrangler's OAuth lacks `dns_records` and Browser MCP can't render the DNS table):
- Registered **`pickle-tech.com`** (old account, ~$10.46/yr — ledger) and moved `prod/staging.pickle.oktryme.com` → `prod/staging.pickle-tech.com` (same IPs, DNS-only).
- Deleted the obsolete records (`_domainconnect`, `bb`/`pallas` tunnels, `blog`, GoDaddy `ns25/ns26`).
- **Kept** the brand-business records (10 total): apex (landing) + `www`; Email Routing MX×3 + SPF + `cf2024-1._domainkey` DKIM (inbound `hello@`/`leads@`/`ops@`); Resend `send` MX + SPF + `resend._domainkey` DKIM (outbound `leads@`). Brand landing serves now via the standalone `oktryme-landing` Worker (route `oktryme.com/*`).

**Previews/QR on `oktryme.com` — ✅ shipped (2026-06-26) via reverse-proxy, no zone move.** The old-account `oktryme-landing` Worker (versioned in the repo at `landing/`) serves the landing at `/` and reverse-proxies `/p/ /r/ /qr/ /convert/ /lead/ /dfy/ /portal/ /img/` → `maps-website-builder.oktrymedigital.workers.dev` (those routes are host-agnostic; the main Worker holds the KV, and the preview HTML uses relative same-origin links so sub-requests proxy too). No zone move, no 30-day lock, no downtime; the dedicated-account security boundary (§5b) is unaffected since customer domains are still registered into the new account. Verified live (`/p/{handle}` renders, `/r/`→`/p/` 302, landing at `/`). `/stripe/webhook` stays direct to the main Worker's `workers.dev` URL. _(The main Worker also serves the landing at its own root — Phase-4 prep — so it's correct if `oktryme.com` is ever attached directly.)_
...
- **Templates:** a small set of responsive static site templates (hero, services, reviews, hours/map, contact). Yellow preview banner + CTA shown only in preview mode. **Built (Phase 2):** one renderer skinned by a per-trade theme (auto / HVAC / landscaping + universal fallback); hero imagery is a CSS-placeholder gradient until real images land — **AI-generated per-trade images via Google Nano Banana Pro (Gemini API)** in Phase 3 (incl. crafting the specialized per-trade prompt), served from R2 at `/img/{key}`. **Template system v2 (Phase 6):** multiple switchable templates per business, a per-category preferred template set, design uplift from external model templates, and an image-provenance guard (never an identifiable picture of another business).
- **Sales CRM (Phase 6):** a per-business **`pipeline_status`** (`new → postcard-sent → qr-code-visit → paid → canceled`) tracking the lead through the sales funnel — distinct from the site-lifecycle `status` and `mail_status`. Auto-advanced by the mail/scan/Stripe signals, with a filterable read view that doubles as the call queue (feeds the §7 #9 scan→call routing).
...
### Product / scope
2. **Template breadth & template-system v2 (Phase 6)** — how many industry templates for a believable MVP across the **allowlisted top trades** (the §1A Step-0 discovery output drives this list)? Phase 6 extends this into **switchable templates + per-category preferred sets**; the **open decision is which TBD external model templates to use as the design reference set** (and the licensing to reuse them). Includes the **image-provenance rule** (no identifiable picture of another business) applied to generated *and* uploaded imagery.
...
- [x] **V1-live — Real domain registration (verified LIVE 2026-06-24):** registered **`assessmybusiness.app`** ($14.20/yr) by driving the **real `CloudflareProvisioner`** end-to-end → zone auto-created + active → Workers Custom Domain attached (apex + `www`) → auto DNS (proxied) → **Universal SSL active in ~30s** → Worker **serves the live site at the apex over valid TLS (HTTP 200, no preview banner)**. WHOIS **redacted** (RDAP shows no registrant); private registrant = **Multiply Technologies LLC**; auto-renew on. **Finding (resolved Phase 2):** `www.` was attached + cert-covered but 404'd — the Worker now sends `www → apex` via a 301 redirect (§5a C.3 spec); see Phase 2. ⚠️ Domain **auto-renews (~$14.20/yr)** and the Registrar lifecycle API can't disable it (§11) — turn off auto-renew / delete in the dashboard if not keeping it.
- [~] **Dedicated account migration (decided 2026-06-24):** account **stood up** — Registrar enabled, scoped token minted, `BUSINESS_KV` created, Worker deployed, read-probe green on the clean account (full detail + rationale in **§5b**). **`oktryme.com` serving:** zone-move **abandoned** (shared multi-service zone); zone **cleaned up 2026-06-26** (pickle → `pickle-tech.com`, junk removed, brand records kept) and stays in the old account — previews to be served via a **reverse-proxy** Worker before Phase 5 (see §5b).
...
| Workers Custom Domain attaches our-account domains + auto DNS/SSL programmatically | ✅ Verified live (V1-live, 2026-06-24) — apex serves over edge SSL; `www→apex` 301 redirect shipped (Phase 2) |
...
| Self-serve AI chat editor edits `business.json` safely | ⏳ Build Phase 7 (schema-constrained, preview/undo) |
...
> **Effort estimates (focused dev-days).** "1 work unit" = the hands-on human time Phase 0 took (~5–6 hrs ≈ **1 focused dev-day**); all estimates below are expressed in those units at the same AI-assisted velocity. Two things shape them: (1) **the spikes are prototypes of later phases** — Phase 0 already wrote and unit-tested the renderer (V4), edit engine (V4), Stripe webhook flow (V3), provisioning client (V1), and email sender (V5), so Phases 2 and 4 are partly done; (2) **everything so far is backend Worker code** — there's no auth or frontend yet, which is why Phase 7 is the outlier. **Total build (Phases 1–7) ≈ 11–16 dev-days (midpoint ~13), ~3 calendar weeks.** The new **Phase 6 overlaps Phase 5** (parallel track), so it adds dev-days but little calendar time. Excluded: the Phase 5a validation test is operational/calendar time (mailing + calling), not dev-days.
...
- [x] **Phase 3 — Category discovery + ingest + copy.** ✅ **Pipeline built & validated end-to-end on live data (2026-06-25).** Keys obtained (Gemini via gcloud CLI on a dedicated `oktryme-maps-website-builder` project w/ a $25 budget alert; Outscraper key via Browser MCP + a $10 prepaid top-up). Decisions locked: **region = Knoxville, TN**; **copy + imagery both via Gemini** (one vendor/key — `gemini-2.5-flash` structured-JSON copy + `gemini-3-pro-image` / Nano Banana Pro for heroes); **validate small first**. Live run: pulled 100 Maps records (auto + HVAC) → **16 no-site survivors (~16% no-site rate)** → Gemini copy → rendered + previewed a real site with a generated hero. 113 tests green. Orchestrated by `npm run ingest -- businesses|images` (`scripts/ingest.ts`). _Operational follow-ons (not dev): the full ~1,000-business Step-0 discovery analysis, larger batches, and the production-R2 hero upload._
  - [x] **Step-0 category discovery** — ✅ **run on a live 1,060-business Knoxville sample (2026-06-27).** Built `scripts/discover.ts` (`npm run discover`): sweeps 28 broad service *sectors* (vs ingest's per-seed-trade query, which can only rediscover what we already listed), de-dupes, and ranks every Google category by no-site rate. Result: 1,060 unique businesses / 115 categories / **12% overall no-site rate**. Findings recommend **+8 off-seed trades** with a real no-site gap: **towing (41% no-site), appliance repair (17%), handyman (16%), locksmith (14%), flooring (13%), pool service (~11%), fencing (9%), concrete (3%)**, plus excluding regulated home-care (Home Instead et al.). **Flagged:** pest control & house cleaning showed a **~0% no-site gap** in Knoxville (saturated) — candidates to drop if other regions agree. The `src/ingest/allowlist.ts` edits applying these are tracked separately; this change ships the reusable discovery harness so the check is repeatable per region. Report + raw cache in `discovery-output/` (gitignored). ~$3.34 prepaid drawdown.
...
- [x] **Phase 4 — Billing + provisioning (§5a).** ✅ **Code shipped & test-mode-verified (2026-06-25).** Stripe → status flip → register domain → Workers Custom Domain (auto DNS+SSL) → live, with a never-refund fallback. 135 tests green; `wrangler deploy --dry-run` clean. **V1-live** (the first paid Registrar registration) was already verified in Phase 0 (§8). _Live config completed (2026-06-25 PM): live-mode Stripe activated; live $49/$99 products+prices (SaaS tax code `txcd_10103001`); Stripe Tax on (WY origin, **no registrations** — SaaS-exempt + no nexus this year, monitoring on); Customer Portal configured (plan switch + cancel + card update); live webhook destination (`/stripe/webhook`, 5 events). Worker (`maps-website-builder`, dedicated account) has the Phase-4 code deployed + **all 10 secrets** (`STRIPE_SECRET_KEY`/`STRIPE_WEBHOOK_SECRET`/`PRICE_*` billing, `CF_*` **real provisioning**, `RESEND_API_KEY`/`LEADS_FROM`/`OPS_FALLBACK_EMAIL` email). Verified: both tiers build live `cs_live_…` Checkout Sessions; webhook rejects unsigned. Remaining go-live steps, in order: the **`oktryme.com` cutover (§5b — now the next Phase-4 item)**, then the first real conversion (**Option B**) — both tracked below._
  ...
  - [x] Done-for-you intake (`POST /dfy/{handle}` → ops email) — bridge until the Phase-7 editor; `src/dfy/intake.ts`
...
- [ ] **Phase 6 — Sales CRM + template system v2.** Can proceed **in parallel with Phase 5** — neither track depends on the postcard send itself, so this is a second workstream alongside outreach. Two independent tracks:
  - **Track A — Sales CRM (funnel-status pipeline; delivers the CRM §7 #9 calls for). ✅ Complete (2026-06-26).** A per-business **sales-funnel status**, kept distinct from the site-lifecycle `status` (preview/active/canceled, §2/§3a) and from `mail_status` — those record *infrastructure* state; this records where the *lead* is in the sales funnel:
    - [x] `pipeline_status` enum on `business.json`: **`new` → `postcard-sent` → `qr-code-visit` → `paid` → `canceled`** (`qr-code-visit` = the lead scanned the QR and opened the preview). `src/types.ts` (`PipelineState` = current status + append-only history); runtime-validated (`src/validate.ts`).
    - [x] Auto-transitions driven by existing signals, idempotent + monotonic (webhook/scan re-delivery never regresses a later stage) — `src/crm/pipeline.ts` (rank-ordered `advancePipeline`; sole sanctioned backward move is reactivation `canceled→paid`). Wired: postcard send / `mail_status=delivered` → `postcard-sent` (`applyMailStatus`, reused by the Phase-5 mail webhook); `/r/{handle}` scan → `qr-code-visit` (`src/index.ts`); Stripe `active` → `paid`, `canceled`/`past_due` → `canceled`, invoice-retry reactivation → `paid` (`src/billing/stripe.ts`). Manual override for offline events (`setPipelineManual`).
    - [x] CRM read view — list/filter businesses by `pipeline_status` with per-handle timestamps/history (`src/crm/view.ts`, `Store.list()`). Token-guarded admin routes (`ADMIN_TOKEN`): `GET /admin/crm` (HTML call queue — hot `qr-code-visit` scanners sorted first, phone as a `tel:` link), `GET /admin/crm.json`, `POST /admin/pipeline/{handle}` (manual override), `POST /admin/mail/{handle}` (record mail status). This is the **call queue** for §1A step 4 and the foundation for the §7 #9 real-time scan→call routing. _(Off the brand domain on purpose — the §5b reverse-proxy doesn't forward `/admin`; reach it on the `workers.dev` URL.)_
  - **Track B — Template system v2 (extends the Phase-2 trade-theme system, §7 #2).**
    - [ ] Raise look & feel using **TBD external model templates** as the reference set — *picking that reference set is an open decision (§7 #2)*; port the chosen designs into the renderer's theme/template layer (one renderer, many skins → no preview/live drift, §3b).
    - [ ] **Switchable templates** — a `template` selector on `business.json` choosing among multiple templates; changing it re-renders preview and live identically.
    - [ ] **Per-category preferred template sets** — each allowlist category (§1A, `src/ingest/allowlist.ts`) maps to a ranked set of templates; ingest applies the category default, an explicit `template` overrides it.
    - [ ] **Image-provenance guard — never an identifiable picture of another business.** Hero/section imagery must not depict a real, identifiable third-party business (storefront, signage/logo, recognizable person). Hardens the existing stance — no Maps photos (§6/§11 #7), generated heroes already avoid text/logos/faces (Phase-3 prompts) — into an explicit check applied to **both** generated and customer-uploaded images.

- [ ] **Phase 7 — AI chat editor (self-serve).** Fast-follow, not funnel-gating: launch managed-first, then ship the editor to cap edit-labor as the base scales. _Build before customer count makes manual edits painful._
