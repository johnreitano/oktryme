...
- [~] **Phase 3 — Category discovery + ingest + copy.** Populate the data store from real Maps data, with copy + imagery. **Pipeline code shipped & unit-tested (2026-06-25); the paid live run is gated on two API keys (Outscraper + Gemini).** Decisions locked: **region = Knoxville, TN metro**; **copy + imagery both via Gemini** (one vendor/key — Gemini text model for copy with structured output, Nano Banana Pro for images); **sequencing = validate small first** (run ~25–50 sample before the full ~1,000 discovery pull). Orchestrated by `npm run ingest -- businesses|images` (`scripts/ingest.ts`); writes a `wrangler kv bulk put` file + R2 image-upload commands (Node has no KV/R2 binding). 104 tests green.
  - [~] **Step-0 category discovery** — **seed allowlist + exclude list shipped & tested** (`src/ingest/allowlist.ts`: 10 Knoxville trades → render themes; chains/poor-fit excluded). The ~1,000-business sample *analysis* that finalizes the list awaits the Outscraper key (§1A).
  - [~] Outscraper ingest (text/factual fields only — no Maps photos/reviews) — `src/ingest/outscraper.ts`: normalizer tested, async-mode live client gated behind `OUTSCRAPER_API_KEY`.
  - [x] Filters — allowlist / no-`site` (social/auto pages count as "no real site") / unambiguous-type (§1A) — `src/ingest/filters.ts`, funnel-counted, tested.
  - [~] AI copy generation + guardrails (verifiable facts only, §7 #5) — `src/copy/generate.ts`: Gemini text model + structured JSON output, no-fabrication system prompt; prompt/parse tested with a mocked fetch, live calls gated behind `GEMINI_API_KEY`.
  ...
  - [~] Batch-generate per-trade images with **Nano Banana Pro** → upload to R2, replacing the Phase-2 CSS placeholders — `src/images/generate.ts`: Gemini client + R2 upload, parsing tested, gated behind `GEMINI_API_KEY`; ingest assigns `trade/{theme}/hero.jpg` so previews show the hero once uploaded (gradient fallback until then).
