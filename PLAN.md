...
- [~] **Phase 3 — Category discovery + ingest + copy.** Populate the data store from real Maps data, with copy + imagery. **Pipeline code shipped & unit-tested (2026-06-25); the paid live run is gated on three API keys (Outscraper / Anthropic / Gemini).** Decisions locked: **region = Knoxville, TN metro**; **copy model = Claude Opus 4.8** (`claude-opus-4-8`, structured outputs); **sequencing = validate small first** (run ~25–50 sample before the full ~1,000 discovery pull). Orchestrated by `npm run ingest -- businesses|images` (`scripts/ingest.ts`); writes a `wrangler kv bulk put` file + R2 image-upload commands (Node has no KV/R2 binding). 104 tests green.
  ...
  - [~] AI copy generation + guardrails (verifiable facts only, §7 #5) — `src/copy/generate.ts`: Claude Opus 4.8 + structured outputs, no-fabrication system prompt; prompt/parse tested with a mocked client, live calls gated behind `ANTHROPIC_API_KEY`.
