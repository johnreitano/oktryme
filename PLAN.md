...
**Remaining (before Phase-5 postcards): serve previews/QR on `oktryme.com` without moving the zone** — expand the old-account `oktryme-landing` Worker into a thin **reverse proxy**: serve the landing at `/` and proxy `/p/ /r/ /qr/ /convert/ /lead/ /dfy/ /img/` → `maps-website-builder.oktrymedigital.workers.dev` (those routes are host-agnostic, and the main Worker holds the KV). No zone move, no 30-day lock, no downtime; the dedicated-account security boundary (§5b) is unaffected since customer domains are still registered into the new account. _(The main Worker already serves the landing at its root — Phase-4 prep — so it's also correct if `oktryme.com` is ever attached directly.)_
...
- [x] **Phase 4 — Billing + provisioning (§5a).** ✅ **Code shipped & test-mode-verified (2026-06-25).** Stripe → status flip → register domain → Workers Custom Domain (auto DNS+SSL) → live, with a never-refund fallback. 135 tests green; `wrangler deploy --dry-run` clean. **V1-live** (the first paid Registrar registration) was already verified in Phase 0 (§8). _Live config completed (2026-06-25 PM): live-mode Stripe activated; live $49/$99 products+prices (SaaS tax code `txcd_10103001`); Stripe Tax on (WY origin, **no registrations** — SaaS-exempt + no nexus this year, monitoring on); Customer Portal configured (plan switch + cancel + card update); live webhook destination (`/stripe/webhook`, 5 events). Worker (`maps-website-builder`, dedicated account) has the Phase-4 code deployed + **all 10 secrets** (`STRIPE_SECRET_KEY`/`STRIPE_WEBHOOK_SECRET`/`PRICE_*` billing, `CF_*` **real provisioning**, `RESEND_API_KEY`/`LEADS_FROM`/`OPS_FALLBACK_EMAIL` email). Verified: both tiers build live `cs_live_…` Checkout Sessions; webhook rejects unsigned. Remaining go-live steps, in order: the **`oktryme.com` cutover (§5b — now the next Phase-4 item)**, then the first real conversion (**Option B**) — both tracked below._
  ...
  - [~] **`oktryme.com` serving (§5b "Phase B") — zone-move abandoned → reverse-proxy.** _Zone cleanup **done (2026-06-26)**: registered `pickle-tech.com` + moved `prod/staging.pickle` off `oktryme.com`, deleted obsolete records (tunnels/blog/GoDaddy), kept brand records (apex+www landing, Email Routing, Resend); landing live via the standalone `oktryme-landing` Worker._ **Remaining:** instead of moving the (shared, multi-service) zone, expand the old-account `oktryme-landing` Worker into a **reverse proxy** — serve landing at `/` and proxy `/p/ /r/ /qr/ /convert/ /lead/ /dfy/ /img/` → `maps-website-builder.oktrymedigital.workers.dev` — so previews/QR resolve on the brand domain with no zone move, no 30-day lock, no downtime (full rationale in §5b). Do before Phase-5 postcards.
  - [ ] **Option B — first real live conversion (deferred; do before/with first real customer).** Everything above is wired & verified in live mode (see the Phase-4 note); the one thing not yet exercised is a real purchase. Run **one real end-to-end conversion** — real Checkout with a real card (~$49/mo to our own Stripe, cancel/refund after) → webhook → `preview→active` → **real Cloudflare domain registration (~$14/yr, auto-renews — needs budget OK)** → custom-domain attach → live over TLS — to prove the full chain in production. Ideally fold this into the **first real Phase-5 postcard lead** so the domain isn't a throwaway. ⚠️ Depends on the **`oktryme.com` cutover above** (the preceding Phase-4 item): until that lands, the temporary `oktryme-landing` Worker catches all `oktryme.com/*`, so preview/QR paths (`/p/`, `/r/`) only resolve on the `maps-website-builder.oktrymedigital.workers.dev` URL.
...
- [ ] **Phase 6 — Sales CRM + template system v2.** Can proceed **in parallel with Phase 5** — neither track depends on the postcard send itself, so this is a second workstream alongside outreach. Two independent tracks:
  - **Track A — Sales CRM (funnel-status pipeline; delivers the CRM §7 #9 calls for).** A per-business **sales-funnel status**, kept distinct from the site-lifecycle `status` (preview/active/canceled, §2/§3a) and from `mail_status` — those record *infrastructure* state; this records where the *lead* is in the sales funnel:
    - [ ] `pipeline_status` enum on `business.json`: **`new` → `postcard-sent` → `qr-code-visit` → `paid` → `canceled`** (`qr-code-visit` = the lead scanned the QR and opened the preview).
    - [ ] Auto-transitions driven by existing signals, idempotent + monotonic (webhook/scan re-delivery never regresses a later stage): postcard send / `mail_status=delivered` → `postcard-sent`; `/r/{handle}` scan log → `qr-code-visit`; Stripe `active` → `paid`; Stripe `canceled`/`past_due` → `canceled`. Manual override allowed for offline events.
    - [ ] CRM read view — list/filter businesses by `pipeline_status` with per-handle timestamps/history. This is the **call queue** for §1A step 4 and the foundation for the §7 #9 real-time scan→call routing.
