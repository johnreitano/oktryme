...
- **Templates:** a small set of responsive static site templates (hero, services, reviews, hours/map, contact). Yellow preview banner + CTA shown only in preview mode. **Built (Phase 2):** one renderer skinned by a per-trade theme (auto / HVAC / landscaping + universal fallback); hero imagery is a CSS-placeholder gradient until real images land — **AI-generated per-trade images via Google Nano Banana Pro (Gemini API)** in Phase 3 (incl. crafting the specialized per-trade prompt), served from R2 at `/img/{key}`.
...
- [x] **Phase 4 — Billing + provisioning (§5a).** ✅ **Code shipped & test-mode-verified (2026-06-25).** Stripe → status flip → register domain → Workers Custom Domain (auto DNS+SSL) → live, with a never-refund fallback. 135 tests green; `wrangler deploy --dry-run` clean. **V1-live** (the first paid Registrar registration) was already verified in Phase 0 (§8). _Live config completed (2026-06-25 PM): live-mode Stripe activated; live $49/$99 products+prices (SaaS tax code `txcd_10103001`); Stripe Tax on (WY origin, **no registrations** — SaaS-exempt + no nexus this year, monitoring on); Customer Portal configured (plan switch + cancel + card update); live webhook destination (`/stripe/webhook`, 5 events). Worker (`maps-website-builder`, dedicated account) has the Phase-4 code deployed + **all 10 secrets** (`STRIPE_SECRET_KEY`/`STRIPE_WEBHOOK_SECRET`/`PRICE_*` billing, `CF_*` **real provisioning**, `RESEND_API_KEY`/`LEADS_FROM`/`OPS_FALLBACK_EMAIL` email). Verified: both tiers build live `cs_live_…` Checkout Sessions; webhook rejects unsigned. Remaining go-live steps, in order: the **`oktryme.com` cutover (§5b — now the next Phase-4 item)**, then the first real conversion (**Option B**) — both tracked below._
  ...
  - [ ] **`oktryme.com` cutover (§5b "Phase B") — ⏭️ NEXT UP.** Move the `oktryme.com` zone from the old personal account into the dedicated account so the brand site + previews/QR serve through the main Worker and the Stripe webhook can move off the `workers.dev` URL onto `oktryme.com`. Procedure (full detail in §5b): release the transfer lock (`clienttransferprohibited`) + disable DNSSEC, verify the registrant email, add it as a website on the new account, submit under **Manage Domain → Configuration** (gaining account approves by email; **30-day transfer lock applies after** — do it in one deliberate window). ⚠️ The zone move does **not** carry DNS records, so re-create them in the new account: **Email Routing** (catch-all + rules), **Resend SPF/DKIM (+ MX)**, and the **preview custom-domain attach** (apex + `www`). Then **fold the temporary `oktryme-landing` page into the main Worker's root (`/`)** and retire the standalone `oktryme-landing` Worker + its `oktryme.com/*` route, so `/p/` `/r/` `/convert/` resolve on the brand domain.
